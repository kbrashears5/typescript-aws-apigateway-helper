...
export class APIGatewayHelper extends BaseClass implements IAPIGatewayHelper {
   ...
   */
  public async CreateApiKeyAsync(
    name: string,
    description: string,
    value?: string,
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(description)) {
      throw new Error(`[${action}]-Must supply description`);
    }
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(value)) {
      params.generateDistinctId = true;
    } else {
      params.value = value;
    }
   ...
   */
  public async CreateUsagePlanAsync(
    name: string,
    description: string,
    ...
    if (!apiStages || apiStages.length === 0) {
      throw new Error(`[${action}]-Must supply at least one apiStage`);
    }
   ...
   */
  public async CreateUsagePlanKeyAsync(
    keyId: string,
    keyType: string,
    usagePlanId: string,
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(keyId)) {
      throw new Error(`[${action}]-Must supply keyId`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(keyType)) {
      throw new Error(`[${action}]-Must supply keyType`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(usagePlanId)) {
      throw new Error(`[${action}]-Must supply usagePlanId`);
    }
  ...
  public async DeleteApiKeyAsync(apiKey: string): Promise<object> {
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(apiKey)) {
      throw new Error(`[${action}]-Must supply apiKey`);
    }
